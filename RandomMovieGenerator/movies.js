const generateBtn = document.getElementById('generate-btn')
    ...
    generateBtn.addEventListener('click', () => {
        setTimeout(() => {
          ...
          const url = `https://api.themoviedb.org/3/discover/movie?api_key=d240c0501df566b063635647bb5d0be5&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=${genre}&vote_average.gte=${minRating}&primary_release_year=${year}`;
          fetch(url)
            .then(res => res.json())
            .then(data => {
